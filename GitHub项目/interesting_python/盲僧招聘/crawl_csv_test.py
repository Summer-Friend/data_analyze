...
def sxs_spider(job, pages):
    ...
    for page in range(1, pages+1):
        ...
        if page == 1:
            basic_data.to_csv(r'E:\vscode_code\GitHub项目\interesting_python\盲僧招聘\{}.csv'.format(job), index=False)  # 保存数据，注意第一页与其它页的区别
        else:
            basic_data.to_csv(r'E:\vscode_code\GitHub项目\interesting_python\盲僧招聘\{}.csv'.format(job), index=False, header=False, mode='a')
...
def com_detailed_data(links):
    ...
    for link in links:
        ...
        # xpath在匹配不到内容的时候会返回空列表，挺不稳定的，很蛋疼，所以定义了一个process_list函数，匹配到返回字符串，匹配不到返回None，下同
